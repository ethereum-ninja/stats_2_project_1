...
sqldf("SELECT PlayerID, Week, NextOpponent, AvgPassDefense, PassingDefense FROM QBCrossSectionalDefensiveOverlayCumulativePassYards WHERE Opponent = 'PHI' ORDER BY Week")


## We need to filter this out for QBs that only played part of a game (limit by pass attempts?) See jets Defense 
## in week 7, matches back to two QBs


sqldf("SELECT
          NextOpponent,
      COUNT(*) FROM QBCrossSectionalDefensiveOverlayCumulativePassYards GROUP BY NextOpponent")

sqldf("SELECT
          * FROM QBCrossSectionalDefensiveOverlayCumulativePassYards WHERE NextOpponent
      IS NULL AND Week <> 17
      ORDER BY PlayerID, Week")
