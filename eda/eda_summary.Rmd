...
for(p in 37:51){
   for(player in seq(1,length(unique(final_ds$PlayerID)),by=30)){
       ...
       player_cum_avg  <- players_ds %>% select(PlayerID,Week,names(players_ds[p])) %>%
                          group_by(PlayerID) %>% arrange(PlayerID,desc(Week)) %>% filter(as.numeric(Week)== max(as.numeric(Week)))
       #player_cum_avg$PlayerID <- as.factor(player_cum_avg$PlayerID)
       bar_plot  <-   player_cum_avg %>% 
                      ggplot(aes_string(x="as.factor(player_cum_avg$PlayerID)",
                                        y=names(player_cum_avg[3]),
                                        fill="as.factor(player_cum_avg$PlayerID)"))+
                      ggtitle(names(player_cum_avg[3]))+
                      geom_bar(stat="identity", width=.5, show.legend = FALSE)+
                      xlab("PlayerID")+
                      ylab(names(players_ds[p]))+  
                      labs(title="Fantasyfootball", 
                           subtitle=names(player_cum_avg[3]), 
                           aption="Source: Fantasyfootball")+
                      theme_economist(base_size=8)+theme(axis.text.y = element_text(size=5))+
                      theme(axis.text.x = element_text(angle=65, vjust=0.6))
           print(bar_plot)
...
## 1.7.8 Scatter plots for cumulative averages

```{r}

for (i in 37:51) {
  Scatter_cumplot <- final_ds %>% 
