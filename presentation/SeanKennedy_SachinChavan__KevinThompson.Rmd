...
return(data %>% 
         filter(Week >= start_week & Week < end_week) %>%        filter(NextWeekFantasyPoints > min_score & NextWeekFantasyPoints < max_score))
}
# 7 = team defense
...
simple_model =  c('NextWeekFantasyPoints', 'AvgPassDefense','HasThrownFoFourTDsOrMore','AvgQBPointsAllowed', 'CumulativeAverageFantasyPoints', 'CumulativeMinFantasyPoints', 'ShortName', 'PlayerID', 'Week')
...
cv_output_lasso = cv.glmnet(train_X, train_Y, 
            alpha = 1, nfolds=100, lambda = lambda_seq, weights = as.numeric(w))
...
cv_output_ridge = cv.glmnet(train_X, train_Y, 
            alpha = 0, nfolds=100, lambda = lambda_seq, weights = as.numeric(w))
...
head(final.result %>% filter(p.value <= 0.1), 10)
