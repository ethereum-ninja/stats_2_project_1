...
anova_data = QBCrossSectional %>% select(-c('NextWeekFantasyPoints', 'NextOpponent')) %>% drop_na()
...
mod = lm(FantasyPoints ~ Week*TeamIsHome, data=anova_data, contrasts = list(Week=contr.sum, 
                                                                                TeamIsHome=contr.sum, 
                                                                                Week=contr.poly, 
                                                                                TeamIsHome=contr.poly))

ggboxplot(anova_data %>% drop_na(), x = "Week", y = "FantasyPoints", color = "TeamIsHome",
...
anova_data = DefensiveStats %>% drop_na()
anova_data$Team = as.factor(anova_data$Team)
anova_data$Week = as.factor(anova_data$Week)


mod = lm(PassingDefense ~ Team, data=anova_data, contrasts = list(              Team=contr.sum, 
                                                                                Team=contr.poly))

ggboxplot(anova_data %>% drop_na(), x = "Team", y = "PassingDefense")+
           theme_wsj()+
           theme(plot.title    = element_text(size = rel(0.5)),
                 plot.subtitle = element_text(size = rel(0.5)),
                 axis.text.x   = element_text(angle=65, vjust=0.6),
